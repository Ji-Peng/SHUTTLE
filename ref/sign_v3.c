/*
 * sign_v3.c - Phase 6c sign / verify using the v3 compressed signature.
 *
 * Math is identical to sign_v2.c (same Alg 2 flow, same mod 2q commitment,
 * same MakeHint/UseHint). The only divergence is on the packing boundary:
 * we call pack_sig_v3 / unpack_sig_v3 which additionally rANS-compress
 * HighBits(z[1..L]) and produce SHUTTLE_BYTES_V3-sized signatures.
 *
 * Keys produced by v3 KeyGen are bit-identical to v2 (same b = a+As+e
 * formula and same pk packing), so a caller can reuse a key pair generated
 * by crypto_sign_keypair_v2.
 */
...
/* ============================================================
 * Helpers (mirror sign_v2.c; duplicated to keep v2/v3 decoupled).
 * ============================================================ */

static void build_sk_full_v3(polyvec *sk_full,
                             const polyvecl *s,
                             const polyveck *e)
{
  unsigned int i, j;
...
static void compute_seed_c_v3(uint8_t seed_c[SHUTTLE_CTILDEBYTES],
                              const polyveck *w_h,
...
static void build_c_eff_v3(poly *c_eff,
...
/* ============================================================
 * Keypair (v3): bit-identical to v2.
 * ============================================================ */
                             ...
                             const uint8_t *sk)
{
  ...
  for(;;) {
    ...
    compute_seed_c_v3(seed_c, &w_h, w_0_bitmap, mu, rho);
    ...
    /* v3: pack with rANS-compressed z1 highs. Reject on OOV / overflow. */
    int rc = pack_sig_v3(sig, seed_c, irs_signs, z_1, &h);
    if(rc != 0) continue;

    *siglen = SHUTTLE_BYTES_V3;
    return 0;
  }
}

/* ============================================================
 * Verify (v3): Alg 3 flow, v3 unpacking.
 * ============================================================ */
                          ...
                          const uint8_t *pk)
{
  ...
  if(siglen != SHUTTLE_BYTES_V3)
    return -1;

  unpack_pk(rho, &b, pk);
  if(unpack_sig_v3(seed_c, irs_signs, z_1, &h, sig))
    return -1;
  ...
  build_c_eff_v3(&c_eff, seed_c, irs_signs);
  ...
  compute_seed_c_v3(seed_c_prime, &w_h, w_0_prime_bitmap, mu, rho);
                   ...
                   const uint8_t *sk)
{
  size_t siglen;
  int ret;
  memmove(sm + SHUTTLE_BYTES_V3, m, mlen);
  ret = crypto_sign_signature_v3(sm, &siglen, sm + SHUTTLE_BYTES_V3, mlen, sk);
  *smlen = siglen + mlen;
  return ret;
}
...
int crypto_sign_open_v3(uint8_t *m, size_t *mlen,
                        ...
                        const uint8_t *pk)
{
  if(smlen < SHUTTLE_BYTES_V3) return -1;
  *mlen = smlen - SHUTTLE_BYTES_V3;
  int ret = crypto_sign_verify_v3(sm, SHUTTLE_BYTES_V3,
                                   sm + SHUTTLE_BYTES_V3, *mlen, pk);
  if(ret) { *mlen = 0; return -1; }
  memmove(m, sm + SHUTTLE_BYTES_V3, *mlen);
  return 0;
}
