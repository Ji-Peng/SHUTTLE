/*
 * test_sign_v3.c - end-to-end tests for Phase 6c (Alg 2 compressed sig, v3).
 *
 * Mirrors test_sign_v2.c but exercises the v3 public entry points, which
 * additionally rANS-compress HighBits(z[1..L]). Target signature sizes:
 *   mode-128: 1560 B  (matches NGCC-Signature Table 2)
 *   mode-256: ~3044 B (above Table 2's 2311; spec clarification pending)
 *
 * Tests:
 *   1. Key generation.
 ...
 *   5. Combined crypto_sign_v3 / crypto_sign_open_v3.
 */
...
int main(void)
{
  ...
  printf("=== SHUTTLE v3 Functional Test ===\n\n");
  printf("Parameters:\n");
  ...
  printf("  Signature:   %d bytes (v3, z1 + hint rANS-compressed)\n", SHUTTLE_BYTES_V3);
  ...
  ret = crypto_sign_keypair_v3(pk, sk);
  ...
  mlen = 33;
  memcpy(msg, "SHUTTLE v3 test message, round 0", mlen);

  ret = crypto_sign_signature_v3(sig, &siglen, msg, mlen, sk);
  if(ret != 0) { printf("  FAIL: sign returned %d\n", ret); return 1; }
  printf("  Signature length: %zu bytes\n", siglen);
  print_hex("  sig", sig, siglen);

  ret = crypto_sign_verify_v3(sig, siglen, msg, mlen, pk);
  ...
  {
    uint8_t sig_bad[SHUTTLE_BYTES_V3];
    memcpy(sig_bad, sig, SHUTTLE_BYTES_V3);
    sig_bad[SHUTTLE_CTILDEBYTES + 10] ^= 0x01;
    ret = crypto_sign_verify_v3(sig_bad, SHUTTLE_BYTES_V3, msg, mlen, pk);
  ...
  }
  {
    ...
    msg_bad[0] ^= 0x80;
    ret = crypto_sign_verify_v3(sig, siglen, msg_bad, mlen, pk);
  ...
  for(i = 0; i < NROUNDS; ++i) {
    ...
    ret = crypto_sign_signature_v3(sig, &siglen, rmsg, rmlen, sk);
    if(ret != 0) { printf("  FAIL at round %u: sign returned %d\n", i, ret); return 1; }
    ret = crypto_sign_verify_v3(sig, siglen, rmsg, rmlen, pk);
  ...
  /* ---- Test 5: Combined API ---- */
  printf("[Test 5] Combined crypto_sign_v3 / crypto_sign_open_v3...\n");
  {
    uint8_t sm[SHUTTLE_BYTES_V3 + MLEN_MAX];
    uint8_t m2[MLEN_MAX];
    size_t smlen, m2len;
    ...
    ret = crypto_sign_v3(sm, &smlen, msg, mlen, sk);
    if(ret != 0) { printf("  FAIL: crypto_sign_v3 returned %d\n", ret); return 1; }
    printf("  Signed message length: %zu bytes\n", smlen);

    ret = crypto_sign_open_v3(m2, &m2len, sm, smlen, pk);
    if(ret != 0) { printf("  FAIL: crypto_sign_open_v3 returned %d\n", ret); return 1; }
